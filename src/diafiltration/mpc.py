...
def _rk4_disc(f, dt):
    ...
    k3 = f(x + 0.5*dt*k2,  u)
    k4 = f(x +     dt*k3,  u)
    return ca.Function("F", [x, u],
                       [x + dt/6*(k1 + 2*k2 + 2*k3 + k4)])

# ───────── Build time-optimal MPC problem ──────────────────────────────
def build_mpc(N: int = 20):
    ...
    for k in range(N):
        ...
        # V_k ≥ 0.0001 m³
        g  += [X[0,k]];           lbg += [1e-4];     ubg += [ca.inf]

        # 0 ≤ u_k ≤ 1
        g  += [U[0,k]];           lbg += [0];        ubg += [1]

        # lactose path constraint
        cL_k = X[1,k] / X[0,k]
        g  += [cL_k];             lbg += [-ca.inf];  ubg += [cL_max]

        J += 1                    # time-optimal cost

    # V_N ≥ 0.0001 m³
    g  += [X[0,N]];               lbg += [1e-4];     ubg += [ca.inf]
